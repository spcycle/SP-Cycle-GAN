...
## CODE FOR MM-WHS SP Cycle-GAN or Default Cycle-GAN DA.
## How to use:
## To train a SP Cycle-GAN model with a specified zeta parameter for the loss function: include 'seggan' and 'gammaX' with X = integers 1-5 to set the zeta value. 
## To specify the DA direction, include MR_to_CT or CT_to_MR, see example of names list below.
## If you dont include 'seggan' in model name, a default Cycle-GAN model will be trained in the specified DA direction. 
## Consult the SegGAN_Dataset_MMWHS function in datasets.py, to ensure you have set the MR and CT directories to the paths you want.

names = ["MR_to_CT_seggan_gamma3","MR_to_CT_default"]

for name in names:
    base_dir = r"D:\Saved_Models\GAN_TrainedModels"
    ...
    if "CT_to_MR" in name:
        ...
        mode = "MRtoCT"
    dataloader = DataLoader(SegGAN_Dataset_MMWHS(transform_image=transforms_image,crop_size = opt.size, mode = mode), batch_size=opt.batchSize, shuffle=True)
    ...
    ##
        ...
        if (epoch+1)%10 == 0:
            ...
            torch.save(netD_A.state_dict(), os.path.join(save_dir,('netD_A_epoch_%s.pth'%str(epoch+1))))
            torch.save(netD_B.state_dict(), os.path.join(save_dir,('netD_B_epoch_%s.pth'%str(epoch+1))))
            if "seggan" in name:
        ...
        torch.save(netD_A.state_dict(), os.path.join(save_dir,('netD_A_current.pth')))
        torch.save(netD_B.state_dict(), os.path.join(save_dir,('netD_B_current.pth')))
        if "seggan" in name:
